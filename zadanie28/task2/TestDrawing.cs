...
namespace Task2
{
    class TestDrawing : Control
    {
        protected override void OnRender(DrawingContext drawingContext)
        {
            ...
            drawingContext.DrawEllipse(Brushes.Black, null, new Point(450, 300), 50, 50);

            drawingContext.DrawRectangle(Brushes.Black, null, new Rect(550, 250, 150, 100));

            DrawPie(drawingContext, Brushes.Black, new Rect(700, 250, 100, 100), 0, 45);
            ...
            for (int i = 0; i < 5; i++)
            {
                drawingContext.DrawRectangle(null, new Pen(Brushes.Black, 1), new Rect(300 + i * offset, i * offset * 2, size, size * 2));
            }
            ...
            for (int row = 0; row < rows; row++)
            {
                for (int col = 0; col < cols; col++)
                {
                    if ((row + col) % 2 == 0)
                    {
                        drawingContext.DrawRectangle(Brushes.Black, null, new Rect(col * tileSize, row * tileSize, tileSize, tileSize));
                    }
                }
            }
        }
